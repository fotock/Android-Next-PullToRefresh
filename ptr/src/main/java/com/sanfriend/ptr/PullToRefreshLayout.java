...
public class PullToRefreshLayout extends ViewGroup {
    ...
    private static final int DEFAULT_RELEASE_TO_REFRESH_SCROLLING_DURATION = 200;
    ...
    private static final int DEFAULT_DEFAULT_TO_REFRESH_SCROLLING_DURATION = 500;
    private static final int DEFAULT_RELEASE_TO_LOAD_MORE_SCROLLING_DURATION = 200;
    ...
    private static final int DEFAULT_DEFAULT_TO_LOAD_MORE_SCROLLING_DURATION = 300;
    ...
    private View mHeaderDecorationView;
     ...
     */
    private int mReleaseToRefreshingScrollingDuration = DEFAULT_RELEASE_TO_REFRESH_SCROLLING_DURATION;
     ...
     */
    private int mDefaultToRefreshingScrollingDuration = DEFAULT_DEFAULT_TO_REFRESH_SCROLLING_DURATION;

    /**
     * <b>ATTRIBUTE:</b>
     * Scrolling duration status release to loading more -> loading more
     */
    private int mReleaseToLoadingMoreScrollingDuration = DEFAULT_RELEASE_TO_LOAD_MORE_SCROLLING_DURATION;
     ...
     */
    private int mDefaultToLoadingMoreScrollingDuration = DEFAULT_DEFAULT_TO_LOAD_MORE_SCROLLING_DURATION;
    ...
    public PullToRefreshLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        final TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.PullToRefreshLayout, defStyleAttr, 0);
        try {
            final int N = a.getIndexCount();
            for (int i = 0; i < N; i++) {
                int attr = a.getIndex(i);
                if (attr == R.styleable.PullToRefreshLayout_refresh_enabled) {
                    setRefreshEnabled(a.getBoolean(attr, true));

                } else if (attr == R.styleable.PullToRefreshLayout_load_more_enabled) {
                    setLoadMoreEnabled(a.getBoolean(attr, true));

                } else if (attr == R.styleable.PullToRefreshLayout_pull_style) {
                    setSwipeStyle(a.getInt(attr, STYLE.CLASSIC));

                } else if (attr == R.styleable.PullToRefreshLayout_refresh_final_drag_offset) {
                    setRefreshFinalDragOffset(a.getDimensionPixelOffset(attr, 0));

                } else if (attr == R.styleable.PullToRefreshLayout_load_more_final_drag_offset) {
                    setLoadMoreFinalDragOffset(a.getDimensionPixelOffset(attr, 0));

                } else if (attr == R.styleable.PullToRefreshLayout_refresh_trigger_offset) {
                    setRefreshTriggerOffset(a.getDimensionPixelOffset(attr, 0));

                } else if (attr == R.styleable.PullToRefreshLayout_load_more_trigger_offset) {
                    setLoadMoreTriggerOffset(a.getDimensionPixelOffset(attr, 0));

                } else if (attr == R.styleable.PullToRefreshLayout_release_to_refresh_scrolling_duration) {
                    setReleaseToRefreshingScrollingDuration(a.getInt(attr, DEFAULT_RELEASE_TO_REFRESH_SCROLLING_DURATION));

                } else if (attr == R.styleable.PullToRefreshLayout_refresh_complete_delay_duration) {
                    setRefreshCompleteDelayDuration(a.getInt(attr, DEFAULT_REFRESH_COMPLETE_DELAY_DURATION));

                } else if (attr == R.styleable.PullToRefreshLayout_refresh_complete_to_default_scrolling_duration) {
                    setRefreshCompleteToDefaultScrollingDuration(a.getInt(attr, DEFAULT_REFRESH_COMPLETE_TO_DEFAULT_SCROLLING_DURATION));

                } else if (attr == R.styleable.PullToRefreshLayout_default_to_refresh_scrolling_duration) {
                    setDefaultToRefreshingScrollingDuration(a.getInt(attr, DEFAULT_DEFAULT_TO_REFRESH_SCROLLING_DURATION));

                } else if (attr == R.styleable.PullToRefreshLayout_release_to_load_more_scrolling_duration) {
                    setReleaseToLoadingMoreScrollingDuration(a.getInt(attr, DEFAULT_RELEASE_TO_LOAD_MORE_SCROLLING_DURATION));

                } else if (attr == R.styleable.PullToRefreshLayout_load_more_complete_delay_duration) {
                    setLoadMoreCompleteDelayDuration(a.getInt(attr, DEFAULT_LOAD_MORE_COMPLETE_DELAY_DURATION));

                } else if (attr == R.styleable.PullToRefreshLayout_load_more_complete_to_default_scrolling_duration) {
                    setLoadMoreCompleteToDefaultScrollingDuration(a.getInt(attr, DEFAULT_LOAD_MORE_COMPLETE_TO_DEFAULT_SCROLLING_DURATION));

                } else if (attr == R.styleable.PullToRefreshLayout_default_to_load_more_scrolling_duration) {
                    setDefaultToLoadingMoreScrollingDuration(a.getInt(attr, DEFAULT_DEFAULT_TO_LOAD_MORE_SCROLLING_DURATION));
     ...
     * the view can also implement {@code PullTrigger} for more extension functions
     *
     * @param view
     */
    public void setRefreshHeaderView(View view) {
        if (view instanceof RefreshHeaderTrigger) {
    ...
    public void setRefreshing(boolean refreshing) {
        ...
        if (refreshing) {
            ...
            int duration;
            int scrollHeight = mHeaderHeight - mHeaderOffset;
            if (mHeaderOffset > mRefreshTriggerOffset) {
                // scroll to RefreshTriggerOffset
                scrollHeight = (int)( mRefreshTriggerOffset- mHeaderOffset );
                duration = mReleaseToRefreshingScrollingDuration;
            } else {
                duration = mDefaultToRefreshingScrollingDuration;
            }
            mAutoScroller.autoScroll(scrollHeight, duration);
        } else {
    ...
    public void setLoadingMore(boolean loadingMore) {
        ...
        if (loadingMore) {
            ...
            int duration;
            int scrollHeight = - mFooterOffset - mFooterHeight;
            if (-mFooterOffset > mLoadMoreTriggerOffset) {
                // scroll to LoadMoreTriggerOffset
                scrollHeight = (int)(- mFooterOffset - mLoadMoreTriggerOffset);
    ...
    protected ViewGroup.LayoutParams generateDefaultLayoutParams() {
        return new PullToRefreshLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT);
    }
    ...
    protected void onFinishInflate() {
        ...
        if (childNum == 0) {
            ...
            mHeaderDecorationView = findViewById(R.id.ptr_header_decoration);
            mHeaderView = findViewById(R.id.ptr_header);
            mBodyView = findViewById(R.id.ptr_body);
            mFooterView = findViewById(R.id.ptr_footer);
        } else {
        ...
        if (mHeaderView != null && mHeaderView instanceof PullTrigger) {
            mHeaderView.setVisibility(GONE);
        }
        if (mFooterView != null && mFooterView instanceof PullTrigger) {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (mHeaderView != null) {
            ...
            int definedHeaderHeight = getResources().getDimensionPixelOffset(R.dimen.refresh_header_height);
            mHeaderHeight = headerView.getMeasuredHeight() + lp.topMargin + lp.bottomMargin;

            // if RefreshTriggerOffset not set, use HeaderHeight
            if (0 == mRefreshTriggerOffset) {
                mRefreshTriggerOffset = mHeaderHeight;
            }
        }
        // target
        if (mBodyView != null) {
            final View targetView = mBodyView;
        ...
        if (mFooterView != null) {
            ...
            // if LoadMoreTriggerOffset not set, use FooterHeight
            if (0 == mLoadMoreTriggerOffset) {
    ...
    protected boolean canChildScrollUp() {
        if (android.os.Build.VERSION.SDK_INT < 14) {
            if (mBodyView instanceof AbsListView) {
                final AbsListView absListView = (AbsListView) mBodyView;
                return absListView.getChildCount() > 0
                        && (absListView.getFirstVisiblePosition() > 0 || absListView.getChildAt(0)
                        .getTop() < absListView.getPaddingTop());
            } else {
                return ViewCompat.canScrollVertically(mBodyView, -1) || mBodyView.getScrollY() > 0;
            }
        } else {
            return ViewCompat.canScrollVertically(mBodyView, -1);
        }
    }
    ...
    private void updateScroll(final float yScrolled) {
        ...
        if (mTargetOffset > 0) {
            mRefreshCallback.onPull(mTargetOffset, STATUS.isRefreshComplete(mStatus));
        } else if (mTargetOffset < 0) {
            mLoadMoreCallback.onPull(mTargetOffset, STATUS.isLoadMoreComplete(mStatus));
        }
        layoutChildren();
        invalidate();
    }
    ...
    RefreshCallback mRefreshCallback = new RefreshCallback() {
        ...
        public void onPrepare() {
            if (mHeaderView != null && mHeaderView instanceof PullTrigger && STATUS.isStatusDefault(mStatus)) {
                mHeaderView.setVisibility(VISIBLE);
                ((PullTrigger) mHeaderView).onPrepare();
            }
        }

        @Override
        public void onPull(int offset, boolean isComplete) {
            if (mHeaderView != null && mHeaderView instanceof PullTrigger && STATUS.isRefreshStatus(mStatus)) {
                if (mHeaderView.getVisibility() != VISIBLE) {
                    mHeaderView.setVisibility(VISIBLE);
                }

                if (mHeaderDecorationView != null && offset - mRefreshTriggerOffset >= 0) {
                    mHeaderDecorationView.getLayoutParams().height = (int)(offset - mRefreshTriggerOffset);
                    mHeaderView.getLayoutParams().height = offset;
                    mHeaderDecorationView.requestLayout();
                    mHeaderView.requestLayout();
                }

                ((PullTrigger) mHeaderView).onPull(offset, isComplete);
            }
        }

        @Override
        public void onRelease() {
            if (mHeaderView != null && mHeaderView instanceof PullTrigger && STATUS.isReleaseToRefresh(mStatus)) {
                if (mHeaderDecorationView != null) {
                    mHeaderDecorationView.getLayoutParams().height = 0;
                    mHeaderView.getLayoutParams().height = mHeaderHeight;
                    mHeaderDecorationView.requestLayout();
                    mHeaderView.requestLayout();
                }
                ((PullTrigger) mHeaderView).onRelease();
            }
        }
        ...
        public void onRefresh() {
            if (mHeaderView != null && STATUS.isRefreshing(mStatus) && mLoading) {
                if (mHeaderView instanceof RefreshHeaderTrigger) {
                    ((RefreshHeaderTrigger) mHeaderView).onRefresh();
                }
        ...
        public void onComplete() {
            if (mHeaderView != null && mHeaderView instanceof PullTrigger) {
                ((PullTrigger) mHeaderView).onComplete();
            }
        }
        ...
        public void onReset() {
            if (mHeaderView != null && mHeaderView instanceof PullTrigger && STATUS.isStatusDefault(mStatus)) {
                if (mHeaderDecorationView != null) {
                    mHeaderDecorationView.setVisibility(VISIBLE);
                    mHeaderDecorationView.requestLayout();
                    mHeaderView.requestLayout();
                }

                ((PullTrigger) mHeaderView).onReset();
                mHeaderView.setVisibility(GONE);
            }
        }
    };
    ...
    LoadMoreCallback mLoadMoreCallback = new LoadMoreCallback() {
        ...
        @Override
        public void onPrepare() {
            if (mFooterView != null && mFooterView instanceof PullTrigger && STATUS.isStatusDefault(mStatus)) {
                mFooterView.setVisibility(VISIBLE);
                ((PullTrigger) mFooterView).onPrepare();
            }
        }

        @Override
        public void onPull(int offset, boolean isComplete) {
            if (mFooterView != null && mFooterView instanceof PullTrigger && STATUS.isLoadMoreStatus(mStatus)) {
                if (mFooterView.getVisibility() != VISIBLE) {
                    mFooterView.setVisibility(VISIBLE);
                }
                ((PullTrigger) mFooterView).onPull(offset, isComplete);
            }
        }

        @Override
        public void onRelease() {
            if (mFooterView != null && mFooterView instanceof PullTrigger && STATUS.isReleaseToLoadMore(mStatus)) {
                ((PullTrigger) mFooterView).onRelease();
            }
        }
        ...
        public void onLoadMore() {
            if (mFooterView != null && STATUS.isLoadingMore(mStatus) && mLoading) {
                if (mFooterView instanceof LoadMoreTrigger) {
                    ((LoadMoreTrigger) mFooterView).onLoadMore();
                }
        ...
        public void onComplete() {
            if (mFooterView != null && mFooterView instanceof PullTrigger) {
                ((PullTrigger) mFooterView).onComplete();
            }
        }
        ...
        public void onReset() {
            if (mFooterView != null && mFooterView instanceof PullTrigger && STATUS.isStatusDefault(mStatus)) {
                ((PullTrigger) mFooterView).onReset();
                mFooterView.setVisibility(GONE);
            }
        }
    };
     ...
     */
    abstract class RefreshCallback implements PullTrigger, RefreshHeaderTrigger {
    }

    /**
     * load more event callback
     */
    abstract class LoadMoreCallback implements PullTrigger, LoadMoreTrigger {
    }
    ...
    private final static class STATUS {
        ...
        private static void printStatus(int status) {
            Log.d(TAG, "ptrStatus:" + getStatus(status));
        }
    }
}
